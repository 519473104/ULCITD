...
class BaseLearner(object):
    ...
    def _evaluate(self, y_pred, y_true):
        ...
        # 关键修改：使用训练前的known_classes作为nb_old
        if self._cur_task == 0:
            nb_old = 0  # 初始任务没有旧类别
        else:
            # 计算当前任务开始时的旧类别数
            nb_old = self._total_classes - self.data_manager.get_task_size(self._cur_task)

        print(f"Debug evaluate: cur_task={self._cur_task}, known_classes={self._known_classes}")
        print(f"Debug evaluate: y_true range: {np.min(y_true)} to {np.max(y_true)}")
        # grouped = accuracy(y_pred.T[0], y_true, self._known_classes)
        grouped = accuracy(y_pred.T[0], y_true, nb_old)
        ret['grouped'] = grouped
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            # k = min(self.topk, outputs.size(1))  # 确保 k 不超过类别数
